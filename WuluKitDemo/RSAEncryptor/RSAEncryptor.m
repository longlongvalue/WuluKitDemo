...
//    NSString *ret = [signData base64EncodedStringWithOptions:NSDataBase64Encoding64CharacterLineLength];
    NSString *finalString = [self dataToHexStringWithData:signData];
...
+ (BOOL)verify:(NSString *)content signature:(NSString *)signature withPublivKey:(NSString *)publicKey {
    ...
    SecKeyRef publicKeyRef = [self addSignPublicKey:publicKey];
    ...
    if (!plainData || !signatureData) { return NO; }


    return [self rsaSHA256VertifyingData:plainData withSignature:signatureData publicKey:publicKeyRef];
}
...
+ (SecKeyRef)addSignPublicKey:(NSString *)pubKey
{
